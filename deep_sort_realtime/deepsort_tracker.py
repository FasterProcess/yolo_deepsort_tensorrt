...
class DeepSort(object):
    def __init__(
        ...
        if embedder is not None:
            ...
            if embedder == "mobilenet_trt":
                from .embedder.mobilenetv2_embedder_trt import (
                    MobileNetv2_Embedder_Trt as Embedder,
                )

                self.embedder = Embedder(
                    model_wts_path=embedding_engine,
                    half=half,
                    max_batch_size=1,
                    bgr=bgr,
                    gpu=embedder_gpu,
                )
            elif embedder == "mobilenet":
    ...
    def push_ctx(self):
        if hasattr(self.embedder, "push_ctx"):
            push_ctx = getattr(self.embedder, "push_ctx")
            push_ctx()

    def pop_ctx(self):
        if hasattr(self.embedder, "pop_ctx"):
            pop_ctx = getattr(self.embedder, "pop_ctx")
            pop_ctx()

    def release(self):
        if hasattr(self.embedder, "release"):
            release = getattr(self.embedder, "release")
            release()
    ...
    def generate_embeds(self, frame, raw_dets, instance_masks=None):
        ...
        if cropped_inst_masks is not None:
            ...
            result = self.embedder.predict(masked_crops)
        else:
            result = self.embedder.predict(crops)
        return result
